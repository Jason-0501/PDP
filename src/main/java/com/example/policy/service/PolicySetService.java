        ...
        return policySetRepository.findById(id).map(policySet -> {
          ...
          for(Policy p : policies) {
        	  ...
        	  if(policy.isPresent()) {
        		  ...
        		  List<Match> matches =  existingPolicy.getTarget().getAnyOf().getAllOfs().stream()  // 展開 allOfs 列表
        				  .flatMap(allOf -> allOf.getMatches().stream())
						  .collect(Collectors.toList()) ; // 收集結果到一個 List
					      // 如果 target 或 anyOf 是 null，返回空列表
        		  for(Match m :matches) {
        			  check.put(m.getDesignater(), m.getAttributeValue());
        		  }
					
				  if(check.get("resource.name")!=null&&!check.get("resource.name").equals(policySetResourceName)) {
						sameResourceName = false;
				  }
				  
			 
				  if ((existingPolicy.getPolicySet() == null || !existingPolicy.getPolicySet().equals(policySet))&&sameResourceName==true) {
	      ...
	      if(sameResourceName == false){
	    	  throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "This policy's resourcename doesn't belong to this policyset");
	      }
